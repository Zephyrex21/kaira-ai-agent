/**
 * Audio handling utility for Kaira Live API Voice stream.
 * Handles:
...
export class KairaAudioSession {
  ...
  public async connect() {
    ...
    try {
      ...
      this.ws.onopen = async () => {
        console.log("[Kaira] Connected to server side WS bridge");
        try {
      ...
      this.ws.onmessage = async (event) => {
        try {
          ...
          if (data.type === "status") {
            console.log("[Kaira WS Status]:", data.status);
          ...
          // Handle interruption signal (e.g. user talked over Kaira)
          ...
          if (data.type === "turnComplete") {
            // Once Kaira completes speaking, change visual state back to listening
            setTimeout(() => {
