"""
KAIRA Desktop Control Agent — FastAPI entrypoint.

Single dispatch endpoint POST /execute { tool, args } -> { result } | { error }.
KAIRA's Node bridge (server.ts) calls this over HTTP on 127.0.0.1:8765.
...
log = logging.getLogger("kaira.desktop")
...
async def lifespan(app: FastAPI):
    log.info("KAIRA Desktop Control Agent v%s starting up.", __version__)
    yield
    ...
    log.info("KAIRA Desktop Control Agent stopped.")


app = FastAPI(
    title="KAIRA Desktop Control Agent",
    version=__version__,
    description="JARVIS-style desktop automation backend for KAIRA.",
    lifespan=lifespan,
)
...
def health() -> Dict[str, Any]:
    return {
        "status": "ok",
        "name": "KAIRA Desktop Control Agent",
        "version": __version__,
...
def main() -> None:
    ...
    host = os.environ.get("KAIRA_AGENT_HOST", "127.0.0.1")
    port = int(os.environ.get("KAIRA_AGENT_PORT", "8765"))
