"""
Windows auto-start management for KAIRA (V2).

Manages a single registry entry under
    HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\\Kaira
which points at the silent launcher batch file (start-kaira-silent.bat) located
...
VALUE_NAME = "Kaira"
SILENT_LAUNCHER = "start-kaira-silent.bat"
...
def _ensure_launcher_exists() -> str:
    """
    Make sure start-kaira-silent.bat exists in the project root.
...
def enable_auto_start(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "result": "Auto-start enabled. Kaira will launch silently on next Windows login.",
        "enabled": True,
        "launcher": launcher,
...
def disable_auto_start(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "result": "Auto-start disabled. Kaira will no longer launch on login.",
        "enabled": False,
    }
...
def get_auto_start_status(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "result": (
            "Auto-start is ENABLED. Kaira launches on Windows login."
            if enabled
