"""
KAIRA Desktop Control Agent — frozen entrypoint.
...
Environment:
    KAIRA_AGENT_HOST   default 127.0.0.1
    KAIRA_AGENT_PORT   default 8765
    KAIRA_DATA_DIR     where logs/ is written (default: cwd)
"""
...
def _resolve_data_dir() -> Path:
    data = os.environ.get("KAIRA_DATA_DIR") or os.getcwd()
    logs = Path(data) / "logs"
    try:
...
def main() -> None:
    ...
    log = logging.getLogger("kaira.agent.boot")

    host = os.environ.get("KAIRA_AGENT_HOST", "127.0.0.1")
    port = int(os.environ.get("KAIRA_AGENT_PORT", "8765"))
    frozen = getattr(sys, "frozen", False)
    log.info("Starting KAIRA agent (frozen=%s) on %s:%d", frozen, host, port)
